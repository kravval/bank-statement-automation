...
Automated processing of ank statements from multiple banks with different formats. The pipeline normalizes column names, merges data into a single table, and classifies each payment by branch, payer type, service category, and collection channel — fully inside Excel, with no VBA or external scripts.
